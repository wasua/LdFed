...
def mnistByzLofed(number_of_samples,iteration_num,hostile_node_percentage,neighbors,is_cnn,is_noniid,is_organized):

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print('             ********** loFed --', device,' **********')

    #number_of_samples = 10 # number of participants
    ...
    for iteration in range(iteration_num):
        ...
        normalNodeList,hostileNodeList=tn.getParameterFrameAndHostileNode(main_model,model_dict,neighbors)

        """
        predictNodeList=[]
        for node in hostileNodeList:
            predictNodeList.append(int(node[5:]))

        print('---byznode:', byzantine_node_list)
        print("---predict:",predictNodeList)

        count=0
        for node in predictNodeList:
            if node in byzantine_node_list:
                count+=1
        predictAcc=count/len(byzantine_node_list)
        predictAccList.append(predictAcc)
        print("************* predict Acc:",predictAcc,"*************")
        """
        main_model=tn.loFed(main_model,model_dict,normalNodeList,device)
    ...
    #test_accuracies_of_each_iteration.insert(0,0)
    return [list(test_accuracies_of_each_iteration),"loFed",test_loss_of_each_iteration,predictAccList]
