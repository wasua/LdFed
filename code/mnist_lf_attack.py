...
def mnistLfAttack(number_of_samples,iteration_num,hostile_node_percentage,is_cnn,is_noniid,is_organized):
    ...
    #iteration_num = 20  ## number of communication rounds
    ...
    node_label_info_test, total_label_occurences_test, amount_info_table_test = \
        dd.get_info_for_distribute_non_iid_with_different_n_and_amount(number_of_samples=number_of_samples,
                                                                       n=n, amount=test_amount, seed=use_seed,
    ...
    lossArray=np.array([],dtype=float)
    ...
    for iteration in range(iteration_num):
        ...
        lossArray=np.append(lossArray,test_loss)

        print("---Iteration", str(iteration + 1), ":accuracy {:7.4f}".format(test_accuracy))
        print("             ", "      loss {:7.4f}".format(test_loss))

    return [list(test_accuracies_of_each_iteration),'attack',list(lossArray)]
