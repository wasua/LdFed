...
def mnistLfLoFed(number_of_samples,iteration_num,hostile_node_percentage,neighbors,is_cnn,is_noniid,is_organized):
    ...
    if is_organized:
        y_train_dict = dd.convert_nodes_to_hostile(y_train_dict,
                            nodes_list,1,10,converter_dict={0: 9, 1: 7, 2: 5, 3: 8, 4: 6, 5: 2, 6: 4, 7: 1, 8: 3,9: 0})
        # converter_dict={0:7,
        #                                                                2:4,
        #                                                                3:6,
        #                                                                4:1,
    ...
    accList,lossList,predictAccList=[],[],[]
    ...
    for iteration in range(iteration_num):
        ...
        normalNodeList, predictHostileNodeList = tn.getParameterFrameAndHostileNode(main_model, model_dict, neighbors)
        a=[]
        for model in predictHostileNodeList:
            a.append(int(model[5:]))
        predictHostileNodeList=sorted(a)

        print('---hostileNode:', hostileNodeList)
        print("---predict byznode:",predictHostileNodeList)

        count=0
        for node in predictHostileNodeList:
            if node in hostileNodeList:
                count+=1
        predictAcc=count/len(hostileNodeList)
    ...
    return [accList, 'loFed', lossList,predictAccList]
