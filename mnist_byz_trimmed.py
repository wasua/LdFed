...
def mnistByzTrimmed(number_of_samples,iteration_num,hostile_node_percentage,is_cnn,is_noniid,is_organized):

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print('             ********** trimmed --',device,' **********')
    ...
    for iteration in range(iteration_num):
        ...
        main_model =  tn.set_trimmed_mean_weights_as_main_model_weights_and_update_main_model(main_model, model_dict, number_of_samples,
    ...
    return [list(test_accuracies_of_each_iteration),"trimmed",test_loss_of_each_iteration]
